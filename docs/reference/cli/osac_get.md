...
```
osac get OBJECT [OPTION]... [ID|NAME]... [flags]
```

### Options

```
      --filter string     CEL expression used for filtering results.
  -h, --help              help for get
      --include-deleted   Include deleted objects.
  -o, --output string     Output format, one of 'table', 'json' or 'yaml'. (default "table")
  -w, --watch             Watch for changes to objects
```
...
* [osac get kubeconfig](osac_get_kubeconfig.md)	 - Get kubeconfig
* [osac get password](osac_get_password.md)	 - Get password
* [osac get token](osac_get_token.md)	 - Shows the authentication token, requesting a new one if necessary
