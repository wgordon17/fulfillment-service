...
```
osac get token [OPTION]... [flags]
```

### Options

```
  -H, --header     Print the token header. This will work only if the token is a JSON web token.
  -h, --help       help for token
  -p, --payload    Shows the token payload in JSON format. This will work only if the token is a JSON web token.
  -r, --refresh    Show the refresh token instead of the access token.
  -R, --rfc-3339   Displays the time claims as RFC 3339 timestamps. By default the time claims are displayed as seconds since the Unix epoch, as that is the format used by JSON web tokens
  -U, --utc        Displays the time claims using the UTC time zone.
```
