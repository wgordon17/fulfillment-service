...
Create a virtual network with the specified network class and IP addressing configuration. At least one of --ipv4-cidr or --ipv6-cidr must be provided.

```
osac create virtualnetwork [flags]
```

### Examples

```
  # Create an IPv4-only virtual network
...
```
  -h, --help                   help for virtualnetwork
      --ipv4-cidr string       IPv4 CIDR block for this network (e.g. 10.0.0.0/16).
      --ipv6-cidr string       IPv6 CIDR block for this network (e.g. fd00::/64).
  -n, --name string            Name of the virtual network.
      --network-class string   Network class to use for this virtual network.
```
